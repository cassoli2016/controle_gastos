 ...
 * (mesmo com provisão futura), aberta fica, e conta que só existe no futuro
 * (o seed de 2030) fica — ela não está paga, está por vir. Totais intactos.
 */
test("paga do mês atual some; futura e aberta ficam; rodapé não muda", async ({ page }) => {
  ...
  await expect(page.getByRole("cell", { name: "ABERTA E2E" })).toBeVisible();
  // Contas só do futuro (seed 2030) não são "pagas": ficam.
