...
export function faturaMonth(dateISO: string, closingDay: number, dueDay?: number | null): string | null {
  ...
  //
  // O dia do fechamento fica PARTIDO entre duas faturas (o banco corta numa
  // hora, não à meia-noite), e nenhum valor de closingDay resolve isso — ver
  // docs/fatura-nubank.md. Por isso esta função é uma aproximação para lançar
  // compra avulsa; quando existe arquivo do banco, quem manda é ele: todas as
  // linhas de um CSV pertencem à fatura daquele arquivo.
